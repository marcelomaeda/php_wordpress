<?php
function pa_iconbox( $attributes, $content = null ) {
	...
	echo '<article id="pa-wrap" class="pa-wrap pa-icon-boxes pa-icon-box' . $type . $iconbox22_class . ' ' . $align . ' ' . $white_color . ' " ' . $iconbox_style . '>';
			if ( $type == 18 ) {
			...
			if( !empty( $icon_link_url) && ($icon_name != 'none') )
				echo '' . $start_wrap . '<a href="' . esc_url($icon_link_url) . '">'  . do_shortcode(  "[pa_icon name='$icon_name' size='$icon_size' color='$icon_color']" ).'</a>' . $end_wrap . '';
			elseif ( $icon_name != 'none' && $type != 21 )
				echo $start_wrap . do_shortcode(  "[pa_icon name='$icon_name' size='$icon_size' color='$icon_color']" ) . $end_wrap;
			if ( !empty( $icon_image ) ) {
			...
			if ( $type == 21 ){
				...
				echo do_shortcode(  "[pa_icon name='$icon_name' size='$icon_size' color='$icon_color']" );
				echo '</div>';
			} 
...
if ( $type != 21 ) {
		...
		echo (!empty($icon_link_url) &&  (!empty($icon_link_text)) )?"<a".$link_style." class=\"pa-magicmore\" href=\"{$icon_link_url}\">{$icon_link_text}</a>":'';
	}
	if ( $type == 21 ) {
		...
		echo '
			<div class="iconbox-rightsection">
				...
				<a'.$link_style.' class="pa-magicmore" href="' . $icon_link_url . '">' . $icon_link_text . '</a>
			</div>';
	}
if ( $type == 18 ) {
	echo '</div>';
...
$out = str_replace('<p></p>','',$out);
 ...
 add_shortcode('pa_iconbox', 'pa_iconbox');