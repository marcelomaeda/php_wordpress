<?php
function vision_webnus_pricing_tables( $attributes, $content = null ) {
	...
	if ( $on_sale_price ) :
		$on_sale_price	= '<h5>' . $on_sale_price . '</h5>';
	endif;
	if ( $type != '6' ) : 
	...
	<div class="pt-footer">
		' . $footer_text . '
		<a href="' . esc_url( $button_url ) . '" class="pa-magicmore">' . esc_html( $button_text ) . '</a>
	</div>';
	...
	$out .= '<div class="w-pricing-table pt-type' . esc_html( $type ) . esc_html( $featured ) . '"' . $pt6_bg_border . '>';
		...
		switch ( $type ) :
			...
			case '7':
				...
				$featured_text = ( $type == 7 && $featured_text && $featured ) ? '<span class="featured-text">' . esc_html( $featured_text ) . '</span>': '';
				$out .= '
				<div class="pt-header">
					' . $featured_text . '
					<h3 class="plan-title">' . esc_html( $title ) . '</h3>
					<h4 class="plan-price"><span>' . $on_sale_price . '</span><span>' . esc_html( $price ) . '</span>' . $period . '</h4>
...
add_shortcode('pa_pricing-tables', 'vision_webnus_pricing_tables');